# -*- coding: utf-8 -*-
"""
Created on Thu Jul  8 10:32:17 2021
...
def get_dataloader(data_root, filename = None, pos_index = [0,1,2], label_index = None, unary_potentials_index = None, features_index = None, sample_size=6000, batch_size=2, log=False):
    """
    ...
    ----------
    data_root : string
        directory where to find the file(s)
    filename : string, optionnal
        name of the file
    ...
    unary_potentials_index : list(int), optionnal
        list of indexes of the class scores. Default is None
    features_index : list(int), optional
    ...
    unary_potentials_index = [int(p) for p in unary_potentials_index] if unary_potentials_index is not None else None
    ...
    for i,file in enumerate(files): 
        with open(file) as f:
            ...
            print("\n \nReading file : ", file)
            print("Content of point cloud \n -------------------------------------")
            print("all columns : ", [ (col,i) for col,i in enumerate(first_line)])                                      #print all available features
            print("selected labels : ", first_line[label_index] if label_index != None else None)                       #print labels used
        ...
        unary_potentials = Tensor(arr[:,unary_potentials_index]) if unary_potentials_index is not None else None   
        
        #Create Data
        data = Data(pos = Tensor(position),  x=features, y=labels,  unary_potentials = unary_potentials, ids=LongTensor(ids)) #convert data to torch_geometric.Data object 
...
if __name__ == '__main__':
    data_root = r'../data'#'D:\InRoads\ascii'
    file = 'segment.txt'#None#r'railway_testV2.txt'
    
    train_loader, weights = get_dataloader(data_root, 
                                           file, 
                                           label_index = None,
                                           features_index = None, 
                                           unary_potentials_index = [13,14,15,16,17],
                                           sample_size=1500,
                                           batch_size=32, 
                                           log=True)