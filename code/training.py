...
from crfasrnn.crfasrnn_model import CrfRnnNet as Net
...
parser.add_argument('--potentials', type=str, help='index of the columns with unary potentials (class score)', default=None)
...
unary_potentials_index =  args.potentials.split(',') if args.potentials is not None else None
...
train_loader, weights = get_dataloader(data_root, train_filename, label_index = labs, features_index = feat_train, unary_potentials_index = unary_potentials_index, sample_size = 1500, batch_size=batch_size, log=True)
...
test_loader, _        = get_dataloader(data_root, test_filename,  label_index = labs, features_index = feat, unary_potentials_index= unary_potentials_index, sample_size = 1500, batch_size=batch_size)
...
for i,l in enumerate(lr):
    ...
    model = Net(num_labels = NUM_CLASSES)
